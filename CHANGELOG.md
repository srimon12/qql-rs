...
- **Contract-driven converter (`qql-convert` crate)**: decodes Qdrant OpenAPI request JSON into the typed `qql-core` AST and renders through `qql_core::fmt` — the same emitter behind `qql fmt` — so every emitted statement re-parses by construction. Covers all 25 statement routes of the statement → endpoint matrix (queries, groups, point retrieval, scroll, count, facet, upserts, deletes, payload/vector mutations, collection/index/shard/quota DDL), with geo predicates lowered onto real `GEO_BBOX` / `GEO_RADIUS` / `GEO_POLYGON` AST. Fields QQL cannot represent fail closed with a typed `ConvertError` (`InvalidJson`, `UnsupportedEndpoint`, `UndecodableBody`, `InvalidField`) and a path instead of emitting placeholder text. `qql convert --collection <name>` names the collection for bare bodies. `tests/contract.rs` asserts covered routes against `openapi.json`; `tests/route_parity.rs` re-plans every converted statement and compares the full REST route (all 13 `QueryExpr` variants, mutations, DDL, `bench/queries.json`).
- **Zero-code-change capture (`qql record`, opt-in `record` feature)**: a transparent proxy that forwards Qdrant REST traffic unchanged (status, headers, auth, query strings) while appending every bodied `/collections/` request as wrapped `{"method","path","body"}` JSONL, plus the converted QQL with `--qql-out`. Point an existing app at `--listen` instead of Qdrant and change nothing else; conversion failures append `# ERROR` annotations and never interrupt forwarding.
