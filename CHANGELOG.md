...
- **Standalone `qql-record` Crate**: Extracted transparent Qdrant REST recorder proxy and live converter from `qql-cli` into standalone workspace member `crates/qql-record` with its own `qql-record` binary CLI, removing `axum` and proxy runtime dependencies from `qql-cli` to keep the main CLI footprint lean; `qql record` transparently delegates to `qql-record` on `$PATH` with installation hints if absent (PR [#164](https://github.com/srimon12/qql-rs/pull/164), commit `c050f6b` `refactor(cli): split qql-record into standalone crate, decouple fastembed, and harden CLI diagnostics & params`).
