...
- **Built-in `qql record` & Dual Release Archives (`qql-*` and `qql-full-*`)**: Integrated the transparent Qdrant REST recorder proxy and live converter directly into `qql-cli` (`qql record`), providing zero-configuration traffic capture without requiring a separate crate or binary installation. GitHub Releases and packaging pipelines now provide both Standard (`qql-*`) and Full (`qql-full-*`, adding local ONNX inference and in-process `qdrant-edge` via `--features full`) prebuilt archives—both extracting the single `qql` command (PR [#164](https://github.com/srimon12/qql-rs/pull/164)).
