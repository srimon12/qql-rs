...
### 📦 Batch `UPDATE … SET VECTOR`
- **One statement, one wire request**: `UPDATE … SET VECTOR` is no longer one point / one name. Compact `SET VECTOR [name] = … WHERE id = …` still covers a single point (including a name map `{dense: …, sparse: …}` on the RHS). `SET VECTOR VALUES {id, vector}, …` is the batch form that maps 1:1 onto REST `PUT /points/vectors` and gRPC `UpdatePointVectors` (`repeated PointVectors`). The planner fills the existing `UpdateVectorRequest.points` list; convert emits one statement instead of fanning out.

### 🔁 REST JSON → QQL Conversion & Traffic Capture (`qql convert` / `qql record`)
- **Contract-driven converter (`qql-convert` crate)**: decodes Qdrant OpenAPI request JSON into the typed `qql-core` AST and renders through `qql_core::fmt` — the same emitter behind `qql fmt` — so every emitted statement re-parses by construction. Covers all 25 statement routes of the statement → endpoint matrix (queries, groups, point retrieval, scroll, count, facet, upserts, deletes, payload/vector mutations, collection/index/shard/quota DDL), with geo predicates lowered onto real `GEO_BBOX` / `GEO_RADIUS` / `GEO_POLYGON` AST. Fields QQL cannot represent fail closed with a typed `ConvertError` (`InvalidJson`, `UnsupportedEndpoint`, `MissingCollection`, `UndecodableBody`, `InvalidField`) and a path instead of emitting placeholder text. One entry `convert(input, collection)` handles a wrapped `{method,path,query?,body?}` request, a bare body, or a JSONL capture; unknown request keys fail closed; `wait` / `timeout` / `consistency` are recovered from the wrapped query string into `WAIT` / `PARAMS`. Bare bodies require an explicit collection (no `FROM unknown`). `tests/contract.rs` asserts covered routes against `openapi.json`; `tests/route_parity.rs` re-plans every converted statement and compares the full REST route including query params (all 13 `QueryExpr` variants, mutations, DDL, `bench/queries.json`).
- **Zero-code-change capture (`qql record`, opt-in `record` feature)**: a transparent proxy that forwards Qdrant REST traffic unchanged (status, headers, auth, query strings) while appending collection and quota requests as wrapped `{"method","path","query?","body?"}` JSONL, plus the converted QQL with `--qql-out`. Bodyless `SHOW` / `DROP` routes and `?wait=` / `?timeout=` / `?consistency=` are captured. Point an existing app at `--listen` instead of Qdrant and change nothing else; conversion failures append `# ERROR` annotations and never interrupt forwarding.
...
- **WASM strict response envelopes**: the WASM client is on the canonical typed response path — every read parses exactly its Qdrant OpenAPI shape (`result.points`, the bare `result` array for `QUERY POINTS`, `result.groups`, `result.count`, facet `result.hits`, collections, shard keys, quota config, `CollectionInfo`) and returns the same JSON as the native SDKs' typed `ExecData` report, failing closed with `QQL-BACKEND-ENVELOPE` on a missing or mistyped field instead of defaulting to an empty or synthesized shape. Batch envelopes and the topology fetch are strict too; server telemetry (`time` / `usage`) stays optional and lenient. The pure shaping layer is host-unit-tested.
...
- **Tunable local BM25 (`k1` / `b` / `avg_len`)**: the client-side document encoder behind sparse `TEXT` / `USING SPARSE` is now configurable through a validated `Bm25Params` type (`k1 > 0`, `b` in `[0, 1]`, `avg_len > 0`, all finite) on every host — Rust `HttpEmbedderOptions` / `qql_edge::{LocalExecutorOptions,FastEmbedderOptions}` / `QqlConfig.bm25_*`, Python `pyqql.HttpEmbedder(...)` and `pyqql_edge.local_executor` / one-shot `execute` / `http_executor` kwargs, Node `{ bm25K1, bm25B, bm25AvgLen }` (`nqql` embedder config and `nqql-edge` local/HTTP executors), CLI `qql config edge --bm25-*` / `QQL_EDGE_BM25_*` (remote CLI via `~/.qql/config.json`), and WASM `client.setBm25Params(k1, b, avgLen)`. Write-path only: query weights stay unit and server-side `qdrant/bm25` inference is untouched; unset stays byte-identical to Qdrant's `1.2 / 0.75 / 256`; invalid values fail closed with `QQL-VALIDATION-CONFIG`; vectors already written keep their weights, so re-ingest to apply (a wrong `avg_len` silently misjudges every document).
...
- **Canonical formatter output aligned with the grammar**: bare vector literals render compact (`QUERY [0.1, 0.2]`, `VECTOR` keyword optional) and `RECOMMEND` example lists render explicit `VECTOR` / `TEXT` / `POINT` inputs. `grammar.pest`, the regenerated parser copy, fixtures, and `Stmt.toString()` follow; a conformance gate now parses every canonical golden under pest so this class of drift cannot return.
- **`qql fmt --check` / `--write` / `qql check` canonicality**: `--check` compared trimmed input against formatter output that ends in a newline and therefore always failed; print added a second newline and `--write` wrote one. `qql check` stage 1 had the same comparison. All now share one canonicality helper; `qql convert` terminates each statement so its output is a canonical script.
...
- **Per-vector `ALTER COLLECTION` diffs**: `ALTER COLLECTION c WITH VECTOR <name> (HNSW (…), QUANTIZATION (…), VECTOR (…))` and `WITH SPARSE <name> (SPARSE (…))` now lower onto typed PATCH `vectors` / `sparse_vectors` maps (REST) and `VectorsConfigDiff` / `SparseVectorConfig` (gRPC) instead of being dropped; the unnamed `WITH VECTOR (…)` form targets the default unnamed vector, unknown names fail fast with `QQL-UNKNOWN-VECTOR`, `datatype` diffs fail closed (`QQL-PARSE-VECTOR-DIFF` / `QQL-PLAN-VECTOR-DIFF`), and the edge backend applies per-vector HNSW while rejecting other per-vector and sparse fields (`QQL-EDGE-UNSUPPORTED-VECTOR-DIFF` / `QQL-EDGE-UNSUPPORTED-SPARSE-DIFF`).
...
- **Typed collection configs on every backend**: collection-level `hnsw`, `optimizer_config`, and `quantization_config` in `SHOW COLLECTION` / dump are plan-owned typed structs — REST parses the config object once, gRPC and edge lower natively with no JSON maps, and the WASM mirror serializes the same types. `null` config sections read as absent (gRPC/edge already omitted them), and edge engine configs classify memory placement through native predicates instead of JSON.
- **Edge text-index stemmer/stopwords**: `CREATE INDEX … TYPE text WITH (stemmer = 'english', stopwords = [...])` now lowers onto qdrant-edge `TextIndexParams` instead of failing closed; unknown stemmer languages still fail with `QQL-EDGE-CONFIG`.
...
- **gRPC `SHOW COLLECTION` status parity**: the proto `CollectionStatus` enum maps to the canonical lowercase names (`green`/`yellow`/`grey`/`red`) instead of leaking the raw proto number (`"1"`), matching REST and edge.
...
- **WAL size knob**: `qql config edge --wal-segment-mb N` / `QQL_EDGE_WAL_SEGMENT_MB` / `LocalExecutorOptions::wal_segment_capacity` shrink the 32 MiB pre-allocated WAL segments (useful for embedded targets); the resolved value persists in `edge_config.json`. The Python (`local_executor(..., wal_segment_mb=N)`) and Node (`localExecutor(dir, { walSegmentMb: N })`) edge SDKs expose the same knob natively (whole MiB ≥ 1; invalid values fail closed with `QQL-VALIDATION-CONFIG`), unlike qdrant-edge 0.8's own Python binding, which has no surface for it.
...
- **Structured `--json` errors for edge commands**: `qql edge optimize --json` and `qql edge bootstrap --json` failures now emit the CLI's `ok`/`operation`/`message` error shape (with the failing `collection`) on stdout while keeping their non-zero exit codes.
- **`qql config edge` patches instead of replacing**: only the flags passed are written; every other key in `edge.json` — known siblings and unknown/future keys alike — is preserved, and the merged configuration is validated as a whole (a persisted `--embed-url` satisfies `--embedder http` on later updates). `--on-disk` / `--no-show-download-progress` flip the two boolean settings back without clobbering siblings.
- **WAL capacity is seed-once**: `QQL_EDGE_WAL_SEGMENT_MB`, `--wal-segment-mb`, and the SDK `wal_segment_capacity` option only seed a shard whose `edge_config.json` has no persisted capacity; the persisted value wins on every later open, so an environment override can no longer ratchet existing collections.
- **`qql edge optimize` reports indexing lag**: the command re-reads `points_count` / `indexed_vectors_count` after the optimizer loop and, when indexing still lags, reports `status: "warn"` / `indexing_lag: true` with the exact counts and the `indexing_threshold` remedy instead of claiming success.
- **Snapshot streaming hardening**: `RemoteSnapshotClient` now rides the shared REST transport (same headers, request ids, and backend error classification) with no total request cap plus a 120 s per-read stall timeout for multi-GB snapshots, and verifies the downloaded byte count against `Content-Length`, failing closed with `QQL-SNAPSHOT-IO` and removing the partial file on a short stream.
