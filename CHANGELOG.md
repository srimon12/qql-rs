...
- **Python DB-API subset** — pure-Python `pyqql.connect()` `Connection`/`Cursor` (`execute`/`executemany`/`fetchone`/`fetchmany`/`fetchall`/iteration, 7-tuple `description`, `rowcount`; `executemany` on `VALUES :rows` delegates to `upsert_many`). `commit()` is a documented no-op, `rollback()`/`callproc`/`setinputsizes` raise `NotSupportedError`. Single hierarchy: native errors *are* the DB-API classes (`except QqlError` still catches everything); mirrored in `pyqql-edge`.
- **Lazy scroll cursor (Node)** — `scrollCursor`/`scrollStream`: memory-bounded async `SCROLL` iteration (one page buffered max) plus pull-driven WHATWG stream; pure JS over `execute`, mirrored in `nqql-edge`.
