...
- **Strict Typed Range Bounds**: Non-scalar RHS values in range filters (objects, arrays, booleans, nulls) now fail closed at parse time with `QQL-PLAN-RANGE-TYPE` instead of generating invalid wire requests (PR [#158](https://github.com/srimon12/qql-rs/pull/158), commit `c75a521` `fix(filters): typed PlanRangeBound, strict range RHS`).
- **CLI Script Non-Zero Exit**: `qql run` now exits with a non-zero exit code whenever any statement fails, ensuring CI/CD scripts correctly halt even in continue-on-error mode (PR [#158](https://github.com/srimon12/qql-rs/pull/158), commit `93abd34` `fix(cli): nonzero exit when any script statement fails`).
- **Strict 64-Bit Integer Validation in JS**: Passing integers beyond `Number.MAX_SAFE_INTEGER` (2⁵³ − 1) as float `Number`s fails closed, requiring explicit `BigInt` to prevent silent truncation (PR [#161](https://github.com/srimon12/qql-rs/pull/161), commit `fa17fbf` `feat: Enhance BigInt support for snowflake IDs and ensure exactness at JS boundary`).
- **Fail-Closed Converter & Planner Invariants**: Unsupported OpenAPI flags return typed `ConvertError` instead of panicking; `SCROLL AFTER` max / UUID-max values are rejected fail-closed; `compile_statement` surfaces serialization errors; and `CROSS RERANK` errors cleanly on non-`Hits` envelopes (PR [#150](https://github.com/srimon12/qql-rs/pull/150), commit `c2173f4` `fix(p0): convert panics, scroll-after, compile-route, cross-rerank envelope`).

### 🚀 Core Engine & Language
- **Wire-Compatible BM25 Text Pipeline**: Bit-for-bit parity document encoder mirroring Qdrant's BM25 tokenization across 30 languages, standard Lucene ASCII folding, stopword lists, and custom vocabulary (PR [#160](https://github.com/srimon12/qql-rs/pull/160), commit `f973989` `feat(embed): Qdrant-compatible BM25 text pipeline`, `9e18d2b` `feat(runtime): BM25 text options + corpus-true avg_len estimator`).
- **Corpus-True BM25 Estimator**: Added `Executor::estimate_bm25_avg_len` to compute authentic average document length directly from target collection samples (PR [#160](https://github.com/srimon12/qql-rs/pull/160), commit `9e18d2b` `feat(runtime): BM25 text options + corpus-true avg_len estimator`).
- **Exact DDL Source Spans**: All collection, field index, and quota validation errors now thread exact source code byte offsets for precise diagnostic reporting (PR [#151](https://github.com/srimon12/qql-rs/pull/151), commit `0961172` `fix(ddl-edge): DDL shard binding, edge rejects, quota spans, wasm deps`; PR [#158](https://github.com/srimon12/qql-rs/pull/158), commit `fe7c82c` `feat(spans): thread source spans into plan validation`; PR [#159](https://github.com/srimon12/qql-rs/pull/159), commit `58e6471` `refactor(plan): span-complete DDL errors, fallible filter lowering`).
- **Score Precision Invariant**: Search scores are rounded once upon ingestion as `f64`, eliminating floating-point widening artifacts (e.g. `0.949999988...` vs `0.95`) (PR [#158](https://github.com/srimon12/qql-rs/pull/158), commit `9e9395c` `fix(scores): round once at ingestion via score_f64`; PR [#159](https://github.com/srimon12/qql-rs/pull/159), commit `e6e1da8` `perf(runtime): stack-buffer score rounding, rank on stored scores`).

### 💻 SDKs & Runtimes (Python, Node, WASM, Edge)
- **Lossless 64-Bit Integers in JS SDKs**: Point IDs (e.g. snowflake `u64`), match counts, and integer payloads crossing the WASM (`qql-wasm`) and Node (`nqql`, `nqql-edge`) boundaries are preserved losslessly as `BigInt` (PR [#161](https://github.com/srimon12/qql-rs/pull/161), commit `fa17fbf`, `dc16210` `feat: enhance handling of BigInt and JSON serialization for u64/i64 values`).
- **Typed Parameter Offloading**: Node SDK executes typed parameters directly via `plan_value_params` with zero-copy `Float32Array` vectors and edge parity (PR [#152](https://github.com/srimon12/qql-rs/pull/152), commit `1790eea` `fix(sdk-typed): Node execute typed params, WASM JSON errors`).
- **WASM Error Classification**: Structured error objects with `.code` and `.fields` attributes for seamless error handling in browser and edge environments (PR [#152](https://github.com/srimon12/qql-rs/pull/152), commit `1790eea` `fix(sdk-typed): Node execute typed params, WASM JSON errors`).
- **Expanded Report Accessors**: Added typed `.collections()`, `.shard_keys()`, and `.quotas()` accessors alongside native-hit `.groups()` across all drivers (PR [#153](https://github.com/srimon12/qql-rs/pull/153), commit `90a65be` `fix(sdk-parity): report accessors, ScoredPoint parity, embedder docs`).
- **Edge Backend Parity & Fail-Closed Guards**: Edge runtime cleanly rejects unsupported `timeout`, `consistency`, and `wait` options with typed error codes (PR [#151](https://github.com/srimon12/qql-rs/pull/151), commit `0961172` `fix(ddl-edge): DDL shard binding, edge rejects, quota spans, wasm deps`).

### 🌐 Website, Playground & DX
- **Built-in `qql record` & Dual Release Artifacts (`qql` and `qql-full`)**: Integrated the transparent Qdrant REST recorder proxy and live converter directly into `qql-cli` (`qql record`), providing zero-configuration traffic capture without requiring a separate crate or binary installation. GitHub Releases and packaging pipelines now provide both Standard (`qql`, lean ~15MB runtime with REST, gRPC, `qql record`, `qql convert`, `qql migrate`, `qql dump`, `qql repl`, `fmt`, `lint`) and Full (`qql-full`, adding local ONNX inference and in-process `qdrant-edge` via `--features full`) prebuilt archives (PR [#164](https://github.com/srimon12/qql-rs/pull/164)).
- **Decoupled FastEmbed from Edge in CLI**: Added independent `--features fastembed` flag in `qql-cli`, enabling zero-server local ONNX embeddings when executing against remote Qdrant clusters or during migrations, without requiring an in-process `qdrant-edge` database (PR [#164](https://github.com/srimon12/qql-rs/pull/164), commit `c050f6b`).
- **Actionable Embedder Diagnostics**: Socket/connection failures (e.g. offline Ollama/vLLM) and queries requiring embeddings with no configured embedder return typed error code `QQL-EMBEDDING-UNAVAILABLE` with clear remediation and installation hints instead of raw TCP socket errors (PR [#164](https://github.com/srimon12/qql-rs/pull/164), commit `c050f6b`).
- **REPL Parameter Ergonomics**: Supported `-p` / `--param` and `--params-file` on `qql repl` CLI startup, and added `\p load <file.json>` / `\p import <file.json>` commands in the interactive REPL for seamless loading of high-dimensional vector parameters without terminal buffer overflows (PR [#164](https://github.com/srimon12/qql-rs/pull/164), commit `c050f6b`).
- **Astro-Native Playground Refactor**: Eliminated 24 loose runtime scripts (~1,700 lines of code removed), rebuilding the playground with idiomatic Astro components and minimal typed modules (PR [#162](https://github.com/srimon12/qql-rs/pull/162), commit `2e39577` `refactor(website): idiomatic Astro playground, remove 24-file scripts sprawl`).
- **Studio-Grade Telemetry Cards**: Polished 2-column metrics dashboard displaying latency, engine time, and compiler metrics without duplication and with unit line-wrapping protection (`whitespace-nowrap`) (PR [#162](https://github.com/srimon12/qql-rs/pull/162), commit `529b5b1` `feat(playground): polish UI cards, resolve i18n warning, and migrate biome`).
- **Direct Documentation Linking**: Support for `?q=` and `?ref=` URL parameters, allowing immediate execution of quickstart queries from docs via "Try in playground ↗" (PR [#162](https://github.com/srimon12/qql-rs/pull/162), commit `529b5b1`).
- **Isolated Deploy CSP Headers**: Section-specific Content-Security-Policy headers preventing policy stacking so the playground can reach custom Qdrant and embedder endpoints (PR [#161](https://github.com/srimon12/qql-rs/pull/161), commit `7a865ae` `fix(website): single-CSP deploy headers, settings defaults cleanup, docs`).
- **Docs Polish & Diagnostics**: Clean documentation with command copy preservation, lossless OG cards, Biome `rules.preset = "recommended"` migration, and resolved Astro `i18n` content collection warnings (PR [#147](https://github.com/srimon12/qql-rs/pull/147), commit `039beaa` `docs(readme): portability-first structure, lint-verified examples`; PR [#148](https://github.com/srimon12/qql-rs/pull/148), commit `b213f69` `website: fix copy shadowing and polish docs layers`, `81e74d6`; PR [#162](https://github.com/srimon12/qql-rs/pull/162), commit `529b5b1`).

### ⚡ Performance & Internal Architecture
- **Single-Pass Planning Core**: Unified query and mutation planning around `plan_and_project`, eliminating redundant planner allocations and intermediate JSON clones (PR [#158](https://github.com/srimon12/qql-rs/pull/158), commit `9ac74be` `refactor(planner): single plan_and_project core`; PR [#159](https://github.com/srimon12/qql-rs/pull/159), commit `672f2c7` `refactor(plan): add RestProjectionError::to_qql_error, delete body_ref`).
- **Batch Embedding Walks**: Batched query embeddings across dense, sparse, and multi-vector models, significantly reducing roundtrips during hybrid search (PR [#154](https://github.com/srimon12/qql-rs/pull/154), commit `88495fe` `fix(perf): batched embeddings, typed REST, owned parses, schema cache`).
- **Topology & Schema Caching**: Cached collection topologies across repeated statement executions to minimize roundtrip metadata calls (PR [#154](https://github.com/srimon12/qql-rs/pull/154), commit `88495fe`).
- **Single HTTP Sender & Method Map**: Consolidated REST transport into a single HTTP sender and unified response envelope parsing, deleting duplicated projection twins (PR [#159](https://github.com/srimon12/qql-rs/pull/159), commit `9cb8875` `refactor(runtime): single HTTP sender, typed alias and exists`, `8062f9d` `refactor(runtime): single Value-envelope path, shared method map`).
...
- **Offline Lint (`qql lint`)**: Syntax plus plan check for files, directories, or inline statements, with `--fix` autofix and `--json` for CI.
- **Lean Feature Installs**: Default binary stays `rest` + `grpc`; `edge` and `record` install from crates.io (`cargo install qql-cli --locked --features edge|record|full`), and `qql version` reports enabled features.
