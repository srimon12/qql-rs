...
| Local BM25 tuning (`k1`/`b`/`avg_len` + text pipeline) | **All SDKs, document side** — Rust `Bm25Params` / `Bm25TextConfig` / `LocalExecutorOptions::bm25_*` / `FastEmbedderOptions` / `HttpEmbedderOptions`, CLI `qql config edge --bm25-*` / `QQL_EDGE_BM25_*`, Python `local_executor(..., bm25_k1=, …, bm25_language=, …)` + `http_executor` + remote `pyqql.HttpEmbedder`, Node `{ bm25K1, …, bm25Language, … }` + remote `embedder`, WASM `setBm25Params(k1, b, avgLen)` + `setBm25Text(…)`. Text pipeline mirrors Qdrant `Bm25Config`: 30 languages, `word`/`whitespace`/`prefix` tokenizers, folding, stopwords/stemmer overrides, length limits. Client-side, write-path only: query weights stay unit, server-side inference untouched; defaults 1.2 / 0.75 / 256, English, word tokenizer; invalid values → `QQL-VALIDATION-CONFIG`; **re-ingest to apply** (not a collection/ALTER setting). `Executor::estimate_bm25_avg_len` measures corpus-true `avg_len` from a field sample |
...
| Client-side BM25 tuning | `Bm25Params::new/1.2,0.75,256-defaults` (`k1 >= 0`) · `Bm25TextConfig` (30 languages, `word`/`whitespace`/`prefix`, folding, stopwords/stemmer, length limits) · `HttpEmbedderOptions.bm25_*` · `qql_edge::{LocalExecutorOptions,FastEmbedderOptions}.bm25_*` · Python `bm25_k1`/`bm25_b`/`bm25_avg_len`/`bm25_language`/… · Node `bm25K1`/`bm25B`/`bm25AvgLen`/`bm25Language`/… · WASM `setBm25Params` + `setBm25Text` · `Executor::estimate_bm25_avg_len` — document-side only, write-path, re-ingest to apply |
