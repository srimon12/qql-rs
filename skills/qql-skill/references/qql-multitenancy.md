...
Numeric keys (`SHARD 101`, `CREATE/DROP SHARD KEY 101 …`, integer
`shard_keys` entries) hash differently from their quoted spellings on the
wire. They stay typed end-to-end — parse → plan → REST/gRPC — on every
statement, so a numeric `SHARD` reaches the numeric partition. `SHARD :tenant`
binds like any other placeholder: strings become keyword keys, non-negative
integers numeric keys (`QQL-BIND-TYPE-MISMATCH` otherwise).
