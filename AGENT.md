...
* **`qql-cli`**: Provides query execution, parser/explain, script splitting, JSON-to-QQL conversion, and live `dump` (`dump_collection` via `QdrantOps::scroll` generating `CREATE COLLECTION`, `CREATE INDEX`, and `INSERT INTO` DDL statements). Supports modular `--no-default-features --features rest` builds for lightweight single-binary deployments (~5.8 MB).
* **Foreign Bindings**: PyO3 (`pyqql`), N-API (`nqql`), and wasm-bindgen (`qql-wasm`) expose parser, tokenization, filter injection, offline compilation, and first-class `Client` and `HttpEmbedder` classes for live Qdrant execution and custom HTTP embedding inference. Keep public class names (`Client`, `HttpEmbedder`), return shapes, and error mappings aligned with crate README files.
...
3. Provide batteries-included runtime adapters behind explicit Cargo features (`default = ["grpc", "rest"]`). The official Rust `qdrant-client` is a gRPC client; REST support is provided by `RestQdrant` (`reqwest`), which is the default lightweight HTTP adapter.
...
5. Bindings share one versioned semantic AST/error schema, exposing first-class `Client` and `HttpEmbedder` classes with native host values (`dict` in Python, JS object in Node) as their ergonomic default. JSON-string APIs (`parseFastJson`) are explicit optimized alternatives.
