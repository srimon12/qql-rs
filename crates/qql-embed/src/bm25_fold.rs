//! ASCII folding (Lucene `ASCIIFoldingFilter` mapping), ported from Qdrant's
...
//! (<https://github.com/qdrant/qdrant/blob/master/LICENSE>). Mapping ported
//! verbatim; only the module docs were adapted.
//!
//! Applied before lowercasing when `ascii_folding` is enabled, exactly like
//! Qdrant's `TokensProcessor` (fold → lowercase → stopwords → stem).
