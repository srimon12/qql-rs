...
pub struct FastEmbedderOptions {
    /// Model name. Accepts enum names (`BGESmallENV15`), HF codes
...
static MODEL_CACHE: OnceLock<Mutex<HashMap<String, Arc<Mutex<TextEmbedding>>>>> = OnceLock::new();
...
impl FastEmbedder {
    pub fn try_new(options: InitOptionsWithLength<EmbeddingModel>) -> Result<Self, QqlError> {
        ...
        let info = TextEmbedding::get_model_info(&options.model_name).map_err(|e| {
            err(format!(
                "fastembed has no model info for '{model_name}': {e}"
            ))
        })?;
        let dim = info.dim;
    ...
    /// Construct from high-level options (model name, cache dir, progress).
    pub fn try_with_options(opts: FastEmbedderOptions) -> Result<Self, QqlError> {
        let model = match opts.model.as_deref() {
            None | Some("") => EmbeddingModel::default(),
            Some(name) => resolve_embedding_model(name)?,
        };
        let mut init = InitOptionsWithLength::new(model);
    ...
    /// Build from a model name string. See [`resolve_embedding_model`].
    ...
    pub fn accepts_model(&self, requested: &str) -> bool {
        ...
        r.eq_ignore_ascii_case(&self.model_name)
            || r.eq_ignore_ascii_case(&self.model_code)
            || short_alias_matches(r, &self.model_code)
    }
}
...
/// List every dense text model fastembed can load.
...
/// Resolve a user-facing model string to an [`EmbeddingModel`].
///
    ...
    async fn embed_dense(&self, text: &str, model: &str) -> Result<Vec<f32>, QqlError> {
        if !self.accepts_model(model) {
            return Err(err(format!(
                "local embedder is locked to model '{}' ({}); cannot satisfy USING MODEL '{model}'. \
