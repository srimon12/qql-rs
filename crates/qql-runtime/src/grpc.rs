...
fn from_grpc_point_id(id: qdrant_client::qdrant::PointId) -> Result<PointId, QqlError> {
    match id.point_id_options {
        ...
        Some(qdrant_client::qdrant::point_id::PointIdOptions::Uuid(uuid)) => Ok(PointId::Uuid(uuid)),
...
fn parse_json_condition(val: &serde_json::Value) -> Result<qdrant_client::qdrant::Condition, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("condition must be a JSON object"))?;
    
    if let Some(is_null) = obj.get("is_null") {
        let key = is_null.get("key").and_then(|v| v.as_str()).unwrap_or_default().to_string();
    ...
    if let Some(is_empty) = obj.get("is_empty") {
        let key = is_empty.get("key").and_then(|v| v.as_str()).unwrap_or_default().to_string();
        return Ok(qdrant_client::qdrant::Condition {
    ...
    if let Some(nested) = obj.get("nested") {
        let key = nested.get("key").and_then(|v| v.as_str()).unwrap_or_default().to_string();
        ...
        if let Some(m) = obj.get("match") {
            let match_value = if let Some(val_inner) = m.get("value") {
                ...
                let match_value = if any_arr.first().and_then(|v| v.as_i64()).is_some() {
                    ...
                    let strings = any_arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect();
                ...
                Some(qdrant_client::qdrant::Match {
                    match_value: Some(qdrant_client::qdrant::r#match::MatchValue::Text(text_val.as_str().unwrap_or_default().to_string())),
                })
            } else if let Some(phrase_val) = m.get("phrase") {
                Some(qdrant_client::qdrant::Match {
                    match_value: Some(qdrant_client::qdrant::r#match::MatchValue::Text(phrase_val.as_str().unwrap_or_default().to_string())),
                })
            } else {
                None
            };
    ...
    Err(QqlError::runtime(format!("unknown condition JSON structure: {:?}", val)))
}

fn to_grpc_filter(filter: crate::backend::Filter) -> Result<qdrant_client::qdrant::Filter, QqlError> {
...
fn to_grpc_vectors(val: &serde_json::Value) -> Result<qdrant_client::qdrant::Vectors, QqlError> {
    ...
    if let Some(arr) = val.as_array() {
        let data: Vec<f32> = arr.iter().filter_map(|v| v.as_f64().map(|f| f as f32)).collect();
        return Ok(qdrant_client::qdrant::Vectors {
            vectors_options: Some(qdrant_client::qdrant::vectors::VectorsOptions::Vector(qdrant_client::qdrant::Vector {
    ...
    if let Some(obj) = val.as_object() {
        ...
        for (name, v_val) in obj {
            if let Some(sparse_obj) = v_val.as_object() {
                if sparse_obj.contains_key("indices") && sparse_obj.contains_key("values") {
                    let indices: Vec<u32> = sparse_obj.get("indices").and_then(|v| v.as_array()).into_iter().flatten().filter_map(|v| v.as_u64().map(|n| n as u32)).collect();
                    let values: Vec<f32> = sparse_obj.get("values").and_then(|v| v.as_array()).into_iter().flatten().filter_map(|v| v.as_f64().map(|f| f as f32)).collect();
                    vectors_map.insert(name.clone(), qdrant_client::qdrant::Vector {
                        vector: Some(qdrant_client::qdrant::vector::Vector::Sparse(qdrant_client::qdrant::SparseVector {
                            indices,
                            values,
                        })),
                        ..Default::default()
        ...
        return Ok(qdrant_client::qdrant::Vectors {
            vectors_options: Some(qdrant_client::qdrant::vectors::VectorsOptions::Vectors(qdrant_client::qdrant::NamedVectors {
                vectors: vectors_map,
            })),
        });
    }
...
#[allow(dead_code)]
fn from_grpc_vector(vec: qdrant_client::qdrant::Vector) -> serde_json::Value {
    if let Some(v) = vec.vector {
        match v {
            qdrant_client::qdrant::vector::Vector::Dense(dense) => {
                serde_json::json!(dense.data)
            }
            qdrant_client::qdrant::vector::Vector::Sparse(sparse) => {
                serde_json::json!({
    ...
    if let Some(v) = vec.vector {
        match v {
            ...
            qdrant_client::qdrant::vector_output::Vector::MultiDense(multi) => {
                let list: Vec<serde_json::Value> = multi.vectors.into_iter().map(|d| serde_json::json!(d.data)).collect();
...
fn from_grpc_scored_point(p: qdrant_client::qdrant::ScoredPoint) -> Result<ScoredPoint, QqlError> {
    let id = p.id.ok_or_else(|| QqlError::runtime("gRPC scored point has no ID"))?;
    ...
    let vector = if let Some(vectors) = p.vectors {
        Some(from_grpc_vectors_output(vectors))
    } else {
        None
    };
...
fn from_grpc_retrieved_point(p: qdrant_client::qdrant::RetrievedPoint) -> Result<RetrievedPoint, QqlError> {
    let id = p.id.ok_or_else(|| QqlError::runtime("gRPC retrieved point has no ID"))?;
    let id = from_grpc_point_id(id)?;
    ...
    let payload = if p.payload.is_empty() {
        ...
        let payload_val = serde_json::to_value(&p.payload).map_err(|e| {
            QqlError::runtime(format!("failed to serialize gRPC payload: {e}"))
        })?;
        Some(serde_json::from_value(payload_val).map_err(|e| {
            QqlError::runtime(format!("failed to deserialize payload: {e}"))
        })?)
    };
...
fn from_grpc_point_group(g: qdrant_client::qdrant::PointGroup) -> Result<PointGroup, QqlError> {
    let id = if let Some(gid) = g.id {
        match gid.kind {
            Some(qdrant_client::qdrant::group_id::Kind::UnsignedValue(val)) => serde_json::json!(val),
            Some(qdrant_client::qdrant::group_id::Kind::IntegerValue(val)) => serde_json::json!(val),
    ...
    let hits: Result<Vec<ScoredPoint>, QqlError> = g
        .hits
        .into_iter()
...
fn to_grpc_vector_input(vi: crate::pipeline::VectorInput) -> qdrant_client::qdrant::VectorInput {
    let variant = match vi {
        crate::pipeline::VectorInput::Id(id) => {
            Some(qdrant_client::qdrant::vector_input::Variant::Id(to_grpc_point_id(id)))
        }
        crate::pipeline::VectorInput::Dense(vec) => {
            Some(qdrant_client::qdrant::vector_input::Variant::Dense(qdrant_client::qdrant::DenseVector {
                data: vec,
            }))
        }
        crate::pipeline::VectorInput::Document { text, model, options } => {
            let grpc_options = options.into_iter().map(|(k, v)| {
                (k, qdrant_client::qdrant::Value {
                    kind: Some(qdrant_client::qdrant::value::Kind::StringValue(v))
                })
            }).collect();
            Some(qdrant_client::qdrant::vector_input::Variant::Document(qdrant_client::qdrant::Document {
                text,
                model,
...
fn to_grpc_search_params(p: crate::pipeline::SearchParams) -> qdrant_client::qdrant::SearchParams {
    qdrant_client::qdrant::SearchParams {
        hnsw_ef: p.hnsw_ef,
        exact: p.exact,
        quantization: p.quantization.map(|q| qdrant_client::qdrant::QuantizationSearchParams {
            ignore: q.ignore,
            rescore: q.rescore,
...
fn to_grpc_lookup_location(lf: crate::pipeline::LookupLocation) -> qdrant_client::qdrant::LookupLocation {
...
fn parse_json_expression(val: &serde_json::Value) -> Result<qdrant_client::qdrant::Expression, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("expression must be a JSON object"))?;
    
    let variant = if let Some(constant) = obj.get("constant") {
        let c = constant.as_f64().ok_or_else(|| QqlError::runtime("constant must be a float"))? as f32;
        qdrant_client::qdrant::expression::Variant::Constant(c)
    } else if let Some(variable) = obj.get("variable") {
        let v = variable.as_str().ok_or_else(|| QqlError::runtime("variable must be a string"))?.to_string();
        qdrant_client::qdrant::expression::Variant::Variable(v)
    } else if let Some(datetime) = obj.get("datetime") {
        let dt = datetime.as_str().ok_or_else(|| QqlError::runtime("datetime must be a string"))?.to_string();
        qdrant_client::qdrant::expression::Variant::Datetime(dt)
    } else if let Some(datetime_key) = obj.get("datetime_key") {
        let dtk = datetime_key.as_str().ok_or_else(|| QqlError::runtime("datetime_key must be a string"))?.to_string();
        qdrant_client::qdrant::expression::Variant::DatetimeKey(dtk)
    } else if let Some(sum) = obj.get("sum") {
        let arr = sum.as_array().ok_or_else(|| QqlError::runtime("sum must be an array"))?;
        let expressions: Result<Vec<qdrant_client::qdrant::Expression>, QqlError> = arr.iter().map(parse_json_expression).collect();
        qdrant_client::qdrant::expression::Variant::Sum(qdrant_client::qdrant::SumExpression { sum: expressions? })
    } else if let Some(mult) = obj.get("mult") {
        let arr = mult.as_array().ok_or_else(|| QqlError::runtime("mult must be an array"))?;
        let expressions: Result<Vec<qdrant_client::qdrant::Expression>, QqlError> = arr.iter().map(parse_json_expression).collect();
        qdrant_client::qdrant::expression::Variant::Mult(qdrant_client::qdrant::MultExpression { mult: expressions? })
        ...
        let pow_obj = pow.as_object().ok_or_else(|| QqlError::runtime("pow must be an object"))?;
        let base = pow_obj.get("base").ok_or_else(|| QqlError::runtime("pow must have base"))?;
        let exponent = pow_obj.get("exponent").ok_or_else(|| QqlError::runtime("pow must have exponent"))?;
        qdrant_client::qdrant::expression::Variant::Pow(Box::new(qdrant_client::qdrant::PowExpression {
        ...
        let gd_obj = geo_distance.as_object().ok_or_else(|| QqlError::runtime("geo_distance must be an object"))?;
        let origin = gd_obj.get("origin").ok_or_else(|| QqlError::runtime("geo_distance must have origin"))?;
        let to = gd_obj.get("to").and_then(|v| v.as_str()).ok_or_else(|| QqlError::runtime("geo_distance must have to"))?.to_string();
        let lat = origin.get("lat").and_then(|v| v.as_f64()).unwrap_or(0.0);
        let lon = origin.get("lon").and_then(|v| v.as_f64()).unwrap_or(0.0);
        qdrant_client::qdrant::expression::Variant::GeoDistance(qdrant_client::qdrant::GeoDistance {
        ...
        let div_obj = div.as_object().ok_or_else(|| QqlError::runtime("div must be an object"))?;
        let left = div_obj.get("left").ok_or_else(|| QqlError::runtime("div must have left"))?;
        let right = div_obj.get("right").ok_or_else(|| QqlError::runtime("div must have right"))?;
        let by_zero_default = div_obj.get("by_zero_default").and_then(|v| v.as_f64()).map(|f| f as f32);
        qdrant_client::qdrant::expression::Variant::Div(Box::new(qdrant_client::qdrant::DivExpression {
        ...
        return Err(QqlError::runtime(format!("unknown expression JSON structure: {:?}", val)));
    };
    
    Ok(qdrant_client::qdrant::Expression { variant: Some(variant) })
}

fn parse_decay_expression(val: &serde_json::Value) -> Result<qdrant_client::qdrant::DecayParamsExpression, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("decay must be an object"))?;
    let x = obj.get("x").ok_or_else(|| QqlError::runtime("decay must have x"))?;
    ...
    let midpoint = obj.get("midpoint").and_then(|v| v.as_f64()).map(|f| f as f32);
...
fn to_grpc_query(variant: crate::pipeline::QueryVariant) -> Result<qdrant_client::qdrant::Query, QqlError> {
    let grpc_variant = match variant {
        ...
        crate::pipeline::QueryVariant::Document { text, model, options } => {
            let grpc_options = options.into_iter().map(|(k, v)| {
                (k, qdrant_client::qdrant::Value {
                    kind: Some(qdrant_client::qdrant::value::Kind::StringValue(v))
                })
            ...
            qdrant_client::qdrant::query::Variant::Nearest(qdrant_client::qdrant::VectorInput {
                variant: Some(qdrant_client::qdrant::vector_input::Variant::Document(
                    qdrant_client::qdrant::Document { text, model, options: grpc_options }
                ))
            })
        }
        crate::pipeline::QueryVariant::Recommend(input) => {
            let positive = input.positive.into_iter().map(to_grpc_vector_input).collect();
            let negative = input.negative.into_iter().map(to_grpc_vector_input).collect();
            ...
            qdrant_client::qdrant::query::Variant::Recommend(qdrant_client::qdrant::RecommendInput {
                positive,
                negative,
                strategy,
            })
        ...
        crate::pipeline::QueryVariant::Context(input) => {
            let pairs = input.pairs.into_iter().map(|p| {
                qdrant_client::qdrant::ContextInputPair {
        ...
        crate::pipeline::QueryVariant::Discover(input) => {
            let context_pairs = input.context.pairs.into_iter().map(|p| {
                qdrant_client::qdrant::ContextInputPair {
                    positive: p.positive.map(to_grpc_vector_input),
                    negative: p.negative.map(to_grpc_vector_input),
                }
        ...
        crate::pipeline::QueryVariant::Sample => {
            qdrant_client::qdrant::query::Variant::Sample(0)
        }
        ...
        crate::pipeline::QueryVariant::Formula { expression, defaults } => {
            let expr = parse_json_expression(&expression)?;
            let mut grpc_defaults = HashMap::new();
            for (key, val) in defaults {
                let val_json = serde_json::json!(val);
                let grpc_val: qdrant_client::qdrant::Value = serde_json::from_value(val_json).map_err(|e| {
                    QqlError::runtime(format!("invalid default value: {e}"))
                })?;
        ...
        crate::pipeline::QueryVariant::RelevanceFeedback(input) => {
            let feedback = input.feedback.into_iter().map(|item| {
                qdrant_client::qdrant::FeedbackItem {
                    example: Some(to_grpc_vector_input(item.example)),
                    score: item.score,
                }
            }).collect();
            let strategy = input.strategy.map(|s| {
                qdrant_client::qdrant::FeedbackStrategy {
            ...
            qdrant_client::qdrant::query::Variant::RelevanceFeedback(qdrant_client::qdrant::RelevanceFeedbackInput {
                target: Some(to_grpc_vector_input(input.target)),
                feedback,
                strategy,
            })
        }
        crate::pipeline::QueryVariant::MMR { input, diversity, candidates } => {
            ...
            qdrant_client::qdrant::query::Variant::NearestWithMmr(qdrant_client::qdrant::NearestInputWithMmr {
    ...
    Ok(qdrant_client::qdrant::Query { variant: Some(grpc_variant) })
}

fn to_grpc_with_payload(wp: &crate::pipeline::WithPayload) -> qdrant_client::qdrant::WithPayloadSelector {
    let selector_options = if !wp.exclude.is_empty() {
        Some(qdrant_client::qdrant::with_payload_selector::SelectorOptions::Exclude(
            qdrant_client::qdrant::PayloadExcludeSelector {
                fields: wp.exclude.clone(),
            }
        ))
    } else if !wp.include.is_empty() {
        Some(qdrant_client::qdrant::with_payload_selector::SelectorOptions::Include(
            qdrant_client::qdrant::PayloadIncludeSelector {
                fields: wp.include.clone(),
            }
        ))
    } else {
        Some(qdrant_client::qdrant::with_payload_selector::SelectorOptions::Enable(
...
fn to_grpc_with_vectors(wv: &crate::pipeline::WithVectors) -> qdrant_client::qdrant::WithVectorsSelector {
    let selector_options = if !wv.vectors.is_empty() {
        Some(qdrant_client::qdrant::with_vectors_selector::SelectorOptions::Include(
            qdrant_client::qdrant::VectorsSelector {
                names: wv.vectors.clone(),
            }
        ))
    } else {
        Some(qdrant_client::qdrant::with_vectors_selector::SelectorOptions::Enable(
...
fn to_grpc_prefetch(pq: crate::pipeline::PrefetchQuery) -> Result<qdrant_client::qdrant::PrefetchQuery, QqlError> {
    let prefetch: Result<Vec<qdrant_client::qdrant::PrefetchQuery>, QqlError> = pq.prefetch
        .into_iter()
        .map(to_grpc_prefetch)
        .collect();
...
fn to_grpc_query_points(req: QueryPointsRequest) -> Result<qdrant_client::qdrant::QueryPoints, QqlError> {
    let prefetch: Result<Vec<qdrant_client::qdrant::PrefetchQuery>, QqlError> = req.prefetch
        .into_iter()
        .map(to_grpc_prefetch)
        .collect();
...
fn to_grpc_query_point_groups(req: QueryPointsGroupsRequest) -> Result<qdrant_client::qdrant::QueryPointGroups, QqlError> {
    let prefetch: Result<Vec<qdrant_client::qdrant::PrefetchQuery>, QqlError> = req.prefetch
        .into_iter()
        .map(to_grpc_prefetch)
        .collect();
    ...
    let with_payload = req.with_payload.map(|wp| to_grpc_with_payload(&wp));
    let with_vectors = req.with_vectors.map(|wv| to_grpc_with_vectors(&wv));
    let with_lookup = req.with_lookup.map(|wl| {
        qdrant_client::qdrant::WithLookup {
...
fn parse_json_vectors_config(val: &serde_json::Value) -> Result<qdrant_client::qdrant::VectorsConfig, QqlError> {
    if let Some(obj) = val.as_object() {
...
fn parse_json_vector_params(val: &serde_json::Value) -> Result<qdrant_client::qdrant::VectorParams, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("vector params must be an object"))?;
    let size = obj.get("size").and_then(|v| v.as_u64()).ok_or_else(|| QqlError::runtime("size is required in vector params"))?;
    let distance_str = obj.get("distance").and_then(|v| v.as_str()).unwrap_or("Cosine");
    let distance = match distance_str.to_lowercase().as_str() {
        "cosine" => 1,
        "dot" => 2,
        "euclid" | "euclidean" => 3,
        "manhattan" => 4,
        _ => return Err(QqlError::runtime(format!("unknown distance: {distance_str}"))),
    };
...
fn parse_json_sparse_vectors_config(val: &serde_json::Value) -> Result<qdrant_client::qdrant::SparseVectorConfig, QqlError> {
    let mut map = HashMap::new();
    if let Some(obj) = val.as_object() {
...
fn parse_json_hnsw_config_diff(val: &serde_json::Value) -> Result<qdrant_client::qdrant::HnswConfigDiff, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("hnsw_config must be an object"))?;
...
fn parse_json_optimizers_config_diff(val: &serde_json::Value) -> Result<qdrant_client::qdrant::OptimizersConfigDiff, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("optimizers_config must be an object"))?;
        ...
        max_optimization_threads: obj.get("max_optimization_threads").and_then(|v| {
            if let Some(n) = v.as_u64() {
                Some(qdrant_client::qdrant::MaxOptimizationThreads {
                    variant: Some(qdrant_client::qdrant::max_optimization_threads::Variant::Value(n)),
                })
            } else if let Some(s) = v.as_str() {
                if s.to_lowercase() == "auto" {
                    Some(qdrant_client::qdrant::MaxOptimizationThreads {
                        variant: Some(qdrant_client::qdrant::max_optimization_threads::Variant::Setting(0)),
                    })
                } else {
                    None
                }
...
fn parse_json_quantization_config(val: &serde_json::Value) -> Result<qdrant_client::qdrant::QuantizationConfig, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("quantization_config must be an object"))?;
    let quantization = if let Some(scalar) = obj.get("scalar") {
        let scalar_obj = scalar.as_object().ok_or_else(|| QqlError::runtime("scalar quantization must be an object"))?;
        let r#type = match scalar_obj.get("type").and_then(|v| v.as_str()).unwrap_or("Int8") {
            "Int8" => 1,
            _ => 1,
        };
        let quantile = scalar_obj.get("quantile").and_then(|v| v.as_f64()).map(|f| f as f32);
        let always_ram = scalar_obj.get("always_ram").and_then(|v| v.as_bool());
        Some(qdrant_client::qdrant::quantization_config::Quantization::Scalar(
        ...
        let product_obj = product.as_object().ok_or_else(|| QqlError::runtime("product quantization must be an object"))?;
        let compression = match product_obj.get("compression").and_then(|v| v.as_str()).unwrap_or("x8") {
            "x8" => 1,
            "x16" => 2,
            "x32" => 3,
            "x64" => 4,
        ...
        Some(qdrant_client::qdrant::quantization_config::Quantization::Product(
...
fn parse_json_quantization_config_diff(val: &serde_json::Value) -> Result<qdrant_client::qdrant::QuantizationConfigDiff, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("quantization_config must be an object"))?;
    let quantization = if let Some(scalar) = obj.get("scalar") {
        let scalar_obj = scalar.as_object().ok_or_else(|| QqlError::runtime("scalar quantization must be an object"))?;
        let r#type = match scalar_obj.get("type").and_then(|v| v.as_str()).unwrap_or("Int8") {
        ...
        let quantile = scalar_obj.get("quantile").and_then(|v| v.as_f64()).map(|f| f as f32);
        let always_ram = scalar_obj.get("always_ram").and_then(|v| v.as_bool());
        Some(qdrant_client::qdrant::quantization_config_diff::Quantization::Scalar(
            qdrant_client::qdrant::ScalarQuantization {
                r#type,
                quantile,
                always_ram,
        ...
        let always_ram = product_obj.get("always_ram").and_then(|v| v.as_bool());
        Some(qdrant_client::qdrant::quantization_config_diff::Quantization::Product(
            qdrant_client::qdrant::ProductQuantization {
                compression,
                always_ram,
            }
...
#[allow(dead_code)]
fn parse_json_collection_params(val: &serde_json::Value) -> Result<qdrant_client::qdrant::CollectionParams, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("params must be an object"))?;
    Ok(qdrant_client::qdrant::CollectionParams {
        shard_number: obj.get("shard_number").and_then(|v| v.as_u64()).map(|n| n as u32).unwrap_or(1),
        replication_factor: obj.get("replication_factor").and_then(|v| v.as_u64()).map(|n| n as u32),
        write_consistency_factor: obj.get("write_consistency_factor").and_then(|v| v.as_u64()).map(|n| n as u32),
        read_fan_out_factor: obj.get("read_fan_out_factor").and_then(|v| v.as_u64()).map(|n| n as u32),
        on_disk_payload: obj.get("on_disk_payload").and_then(|v| v.as_bool()).unwrap_or(false),
        vectors_config: None,
...
fn parse_json_collection_params_diff(val: &serde_json::Value) -> Result<qdrant_client::qdrant::CollectionParamsDiff, QqlError> {
    let obj = val.as_object().ok_or_else(|| QqlError::runtime("params must be an object"))?;
...
impl QdrantOps for GrpcQdrant {
    async fn list_collections(&self) -> Result<Vec<String>, QqlError> {
        let resp = self.client.list_collections().await.map_err(|e| {
            QqlError::runtime(format!("gRPC list_collections failed: {e}"))
        })?;
    ...
    async fn collection_exists(&self, name: &str) -> Result<bool, QqlError> {
        self.client.collection_exists(name).await.map_err(|e| {
            QqlError::runtime(format!("gRPC collection_exists failed: {e}"))
        })
    }

    async fn get_collection_info(&self, name: &str) -> Result<CollectionInfo, QqlError> {
        let resp = self.client.collection_info(name).await.map_err(|e| {
            QqlError::runtime(format!("gRPC collection_info failed: {e}"))
        })?;
        ...
        let quantization_kind = if let Some(ref config) = info.config {
            if let Some(ref qc) = config.quantization_config {
                if let Some(ref q) = qc.quantization {
                    match q {
                        qdrant_client::qdrant::quantization_config::Quantization::Scalar(_) => Some("scalar"),
                        qdrant_client::qdrant::quantization_config::Quantization::Product(_) => Some("product"),
                        qdrant_client::qdrant::quantization_config::Quantization::Binary(_) => Some("binary"),
                        _ => Some("scalar"),
                    }
                } else {
                    None
    ...
    async fn create_collection(&self, req: CreateCollectionReq) -> Result<(), QqlError> {
        let vectors_config = req.vectors_config
            .map(|v| parse_json_vectors_config(&v))
            .transpose()?;
            
        let sparse_vectors_config = req.sparse_vectors_config
            .map(|v| parse_json_sparse_vectors_config(&v))
            .transpose()?;
            
        let hnsw_config = req.hnsw_config
            .map(|v| parse_json_hnsw_config_diff(&v))
            .transpose()?;
            
        let optimizers_config = req.optimizers_config
            .map(|v| parse_json_optimizers_config_diff(&v))
            .transpose()?;
            
        let quantization_config = req.quantization_config
        ...
        if let Some(p) = req.params {
            if let Some(obj) = p.as_object() {
                shard_number = obj.get("shard_number").and_then(|v| v.as_u64()).map(|n| n as u32);
                on_disk_payload = obj.get("on_disk_payload").and_then(|v| v.as_bool());
                replication_factor = obj.get("replication_factor").and_then(|v| v.as_u64()).map(|n| n as u32);
                write_consistency_factor = obj.get("write_consistency_factor").and_then(|v| v.as_u64()).map(|n| n as u32);
            }
        ...
        self.client.create_collection(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC create_collection failed: {e}"))
        })?;
        Ok(())
    }

    async fn update_collection(&self, req: serde_json::Value) -> Result<(), QqlError> {
        let collection_name = req.get("collection_name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| QqlError::runtime("collection_name is required"))?
            .to_string();
            
        let optimizers_config = req.get("optimizers_config")
            .map(|v| parse_json_optimizers_config_diff(v))
            .transpose()?;
            
        let hnsw_config = req.get("hnsw_config")
            .map(|v| parse_json_hnsw_config_diff(v))
            .transpose()?;
            
        let quantization_config = req.get("quantization_config")
            .map(|v| parse_json_quantization_config_diff(v))
            .transpose()?;
            
        let params = req.get("params")
            .map(|v| parse_json_collection_params_diff(v))
            .transpose()?;
        ...
        self.client.update_collection(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC update_collection failed: {e}"))
        })?;
        Ok(())
    }
    ...
    async fn delete_collection(&self, name: &str) -> Result<(), QqlError> {
        self.client.delete_collection(name).await.map_err(|e| {
            QqlError::runtime(format!("gRPC delete_collection failed: {e}"))
        })?;
        Ok(())
    }
    ...
    async fn upsert(&self, req: UpsertPointsReq) -> Result<(), QqlError> {
        let points: Result<Vec<qdrant_client::qdrant::PointStruct>, QqlError> = req
            ...
            .map(|point| {
                ...
                let payload: qdrant_client::Payload = serde_json::from_value(serde_json::Value::Object(
        ...
        self.client.upsert_points(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC upsert failed: {e}"))
        })?;
        Ok(())
    }
    ...
    async fn query(&self, req: QueryPointsRequest) -> Result<Vec<ScoredPoint>, QqlError> {
        ...
        let resp = self.client.query(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC query failed: {e}"))
        })?;
    ...
    async fn query_groups(
        ...
        let resp = self.client.query_groups(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC query_groups failed: {e}"))
        })?;
    ...
    async fn query_batch(
        ...
        let resp = self.client.query_batch(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC query_batch failed: {e}"))
        })?;
    ...
    async fn delete(&self, req: DeletePointsReq) -> Result<(), QqlError> {
        let points_selector = if let Some(id) = req.point_id {
            ...
            Some(qdrant_client::qdrant::PointsSelector {
                points_selector_one_of: Some(qdrant_client::qdrant::points_selector::PointsSelectorOneOf::Filter(
        ...
        self.client.delete_points(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC delete failed: {e}"))
        })?;
        Ok(())
    }
    ...
    async fn update_vectors(&self, req: UpdateVectorsReq) -> Result<(), QqlError> {
        ...
        self.client.update_vectors(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC update_vectors failed: {e}"))
        })?;
        Ok(())
    }
    ...
    async fn set_payload(&self, req: SetPayloadReq) -> Result<(), QqlError> {
        ...
        for (key, val) in req.payload {
            let val_json = serde_json::json!(val);
            let grpc_val: qdrant_client::qdrant::Value = serde_json::from_value(val_json).map_err(|e| {
                QqlError::runtime(format!("invalid payload value: {e}"))
            })?;
        ...
        self.client.set_payload(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC set_payload failed: {e}"))
        })?;
        Ok(())
    }
    ...
    async fn create_field_index(&self, req: CreateFieldIndexReq) -> Result<(), QqlError> {
        ...
        let field_type = match req.field_type.to_lowercase().as_str() {
            ...
            _ => return Err(QqlError::runtime(format!("unknown field type: {}", req.field_type))),
        } as i32;

        let field_index_params = if !options.is_empty() {
            let index_params = match req.field_type.to_lowercase().as_str() {
                "text" => {
                    let tokenizer_str = options.get("tokenizer").and_then(|v| v.as_str()).unwrap_or("word");
                    ...
                    Some(qdrant_client::qdrant::payload_index_params::IndexParams::TextIndexParams(
                ...
                "float" => {
                    let on_disk = options.get("on_disk").and_then(|v| v.as_bool());
                    Some(qdrant_client::qdrant::payload_index_params::IndexParams::FloatIndexParams(
                ...
                "geo" => {
                    let on_disk = options.get("on_disk").and_then(|v| v.as_bool());
                    Some(qdrant_client::qdrant::payload_index_params::IndexParams::GeoIndexParams(
                ...
                "bool" => {
                    let on_disk = options.get("on_disk").and_then(|v| v.as_bool());
                    Some(qdrant_client::qdrant::payload_index_params::IndexParams::BoolIndexParams(
                ...
                "uuid" => {
                    let on_disk = options.get("on_disk").and_then(|v| v.as_bool());
                    Some(qdrant_client::qdrant::payload_index_params::IndexParams::UuidIndexParams(
        ...
        self.client.create_field_index(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC create_field_index failed: {e}"))
        })?;
        Ok(())
    }
    ...
    async fn scroll(
        ...
        let resp = self.client.scroll(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC scroll failed: {e}"))
        })?;
        ...
        let next_page_offset = resp
            .next_page_offset
            .map(from_grpc_point_id)
            .transpose()?;
    ...
    async fn count(&self, req: CountPointsReq) -> Result<u64, QqlError> {
        ...
        let resp = self.client.count(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC count failed: {e}"))
        })?;
    ...
    async fn get(&self, req: GetPointsReq) -> Result<Vec<RetrievedPoint>, QqlError> {
        ...
        let resp = self.client.get_points(grpc_req).await.map_err(|e| {
            QqlError::runtime(format!("gRPC get failed: {e}"))
        })?;
