//! Conversions for the typed semantic primitives.
//!
//! The type definitions live in [`crate::semantic`]; this module holds every
//! `Display` / `From` / `Serialize` / `Deserialize` conversion, the
//! [`PlanVectorValue::from_value`] constructor, and the private serialization
//! helpers. The split from `semantic.rs` is mechanical (size hygiene) —
...
use crate::semantic::{
    PlanPointId, PlanPointVectors, PlanQueryInput, PlanShardKey, PlanVectorStruct, PlanVectorValue,
};
