...
| 1.7 | 2026-09-06 | Additive minor syntax: formal parameter placeholders in grammar (`param = { ":" ~ identifier | "?" }`) across scalars, point IDs, query inputs, and clauses; prepared statement AST binding (`bind_stmt`), scoped batch parameter maps, and nested property parameter expansion; optional trailing `WAIT true|false` durability clause on UPSERT, DELETE, CLEAR PAYLOAD, DELETE PAYLOAD, DELETE VECTOR, UPDATE … VECTOR, UPDATE … PAYLOAD, and CREATE INDEX (canonical order after `SHARD`; duplicate `WAIT` rejected with `QQL-PARSE-DUPLICATE-CLAUSE`, new code for a previously unspecified case). Conformance corpus 40 valid files (285 statements), 63 invalid cases, 40 AST snapshots, 40 canonical formats. |
