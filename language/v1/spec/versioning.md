...
| 1.7 | 2026-09-06 | Additive minor syntax (unreleased): formal parameter placeholders in grammar (`param = { ":" ~ identifier | "?" }`) across scalars, point IDs, query inputs, and clauses (including `SHARD` routing keys, bound via `bind_stmt` like any other placeholder); prepared statement AST binding (`bind_stmt`), scoped batch parameter maps, and nested property parameter expansion; optional trailing `WAIT true|false` durability clause on UPSERT, DELETE, CLEAR PAYLOAD, DELETE PAYLOAD, DELETE VECTOR, UPDATE … VECTOR, UPDATE … PAYLOAD, and CREATE INDEX (canonical order after `SHARD`; duplicate `WAIT` rejected with `QQL-PARSE-DUPLICATE-CLAUSE`, new code for a previously unspecified case). Typed shard keys end-to-end: `SHARD 101`, `CREATE/DROP SHARD KEY 101`, and integer `shard_keys` entries keep numeric form to the wire (keyword and number keys hash differently); `DROP SHARD KEY` accepts quoted or numeric keys. `ALTER COLLECTION` per-vector diffs: named/unnamed `WITH VECTOR <name> (HNSW (…), QUANTIZATION (…), VECTOR (…))` and `WITH SPARSE <name> (SPARSE (…) \| INDEX (…))` (strict blocks, `datatype` rejected with `QQL-PARSE-VECTOR-DIFF` / `QQL-PLAN-VECTOR-DIFF`, names validated via `QQL-UNKNOWN-VECTOR`; edge applies per-vector HNSW only: `QQL-EDGE-UNSUPPORTED-VECTOR-DIFF` / `-SPARSE-DIFF`). Conformance corpus 40 valid files (292 statements), 66 invalid cases, 40 AST snapshots, 40 canonical formats. |
