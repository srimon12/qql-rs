...
| 1.7 | 2026-09-06 | Placeholders (`param = { ":" ~ identifier \| "?" }`) across scalars, point IDs, query inputs and clauses incl. `SHARD` routing keys (`bind_stmt`, scoped batch maps, nested expansion); trailing `WAIT true\|false` on UPSERT, DELETE, CLEAR/DELETE PAYLOAD, DELETE VECTOR, UPDATE VECTOR/PAYLOAD and CREATE INDEX (after `SHARD`; dup rejected `QQL-PARSE-DUPLICATE-CLAUSE`); typed shard keys end-to-end (`SHARD 101`, `CREATE`/`DROP SHARD KEY 101`, int `shard_keys` keep numeric form); `ALTER COLLECTION` per-vector diffs (`WITH VECTOR [name] (HNSW, QUANTIZATION, VECTOR)`, `WITH SPARSE name (SPARSE \| INDEX)`, strict blocks, `datatype` rejected `QQL-PARSE-VECTOR-DIFF` / `QQL-PLAN-VECTOR-DIFF`, `QQL-UNKNOWN-VECTOR`, edge HNSW-only `QQL-EDGE-UNSUPPORTED-VECTOR-DIFF` / `-SPARSE-DIFF`); canonical-form additions: bare vector literals in query inputs (`QUERY [0.1, 0.2]` — `VECTOR` keyword optional) and full query inputs in `RECOMMEND` example lists (`point_id \| query_input`, bare strings/integers stay point IDs). `UPDATE … SET VECTOR` is a point list: compact `WHERE id =` (RHS may be a named vector map) and batch `VALUES {id, vector}, …` matching REST `PUT /points/vectors` / gRPC `UpdatePointVectors`; AST field is `points` (`QQL-VALIDATION-UPDATE-VECTOR`). First-class `BATCH { … }` blocks with header `WAIT` / `PARAMS` (one RPC per block, homogeneous collection and family); `MIN SHOULD`, `MATCH TOKENS`, `MATCH EXCEPT`, `u64` literals, string range bounds; `SCROLL … ORDER BY` and payload selectors, `ORDER BY … START FROM`, full `LOOKUP` selectors, `lookup_from` shard routing; upsert `UPDATE FILTER` / `UPDATE MODE`, payload `KEY` and `OVERWRITE`; inference `OPTIONS` and `OBJECT` inputs, per-point inference vectors; text-index stopwords/languages, collection `WAL` / `STRICT_MODE` / `METADATA` / read-fan-out / placement blocks; canonical `DATETIME(…)` / `DATETIME_KEY(…)` casing. Corpus 41 files (300 statements), 73 invalid, 41 snapshots, 41 formats. |
