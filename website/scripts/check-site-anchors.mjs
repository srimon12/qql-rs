...
function targetFile(urlPath) {
  ...
  return candidates.find((candidate) => candidate.endsWith(".html") && existsSync(candidate));
}
...
for (const file of htmlFiles) {
  ...
  for (const match of html.matchAll(/href="([^"]+)"/g)) {
    ...
    if (!target) {
      failures.push({ file: relative, message: `link "${href}" targets a page that does not exist in dist` });
      continue;
    }
...
for (const { file, href, fragment, target } of fragmentLinks) {
  ...
  if (!targetHtml.includes(`id="${fragment}"`)) {
    failures.push({ file, message: `link "${href}" targets fragment #${fragment}, which has no id on ${target.slice(distRoot.length + 1)}` });
  }
}
