...
All public packages use one repository version. The current release is `0.3.2`;
the corresponding Git tag is `v0.3.2`. The QQL language specification version
...
| VS Code Marketplace | `srimon12.qql-lang` (extension version is independent from the workspace release; currently `0.3.0`) |
...
7. Validate synchronized metadata:
   ...
   ```bash
   python3 scripts/check_release.py --version 0.3.2
   ```
...
```bash
cargo info --registry crates-io qql-core@0.3.2
cargo info --registry crates-io qql@0.3.2
cargo info --registry crates-io qql-edge@0.3.2
cargo install qql-cli@0.3.2 --locked --features edge

python -m pip install pyqql==0.3.2
python -m pip install pyqql-edge==0.3.2

npm view @veristamp/nqql@0.3.2
npm view @veristamp/nqql-edge@0.3.2
npm view qql-wasm@0.3.2
```
