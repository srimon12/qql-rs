...
All public packages use one repository version. The current release is `0.4.2`;
the corresponding Git tag is `v0.4.2`. The QQL language specification version
...
| VS Code Marketplace | `srimon12.qql-lang` (extension version matches the workspace release; currently `0.4.2`) |
...
2. Synchronize every version site from the single source of truth (`VERSION`):
   ...
   ```bash
   python3 scripts/check_release.py set 0.4.2
   ```
...
4. Move the `[Unreleased]` notes in `CHANGELOG.md` into a `[0.4.2]` section
...
8. Validate synchronized metadata:
   ...
   ```bash
   python3 scripts/check_release.py --version 0.4.2
   ```
...
```bash
cargo info --registry crates-io qql-core@0.4.2
cargo info --registry crates-io qql-convert@0.4.2
cargo info --registry crates-io qql@0.4.2
cargo info --registry crates-io qql-edge@0.4.2
cargo install qql-cli@0.4.2 --locked --features edge

python -m pip install pyqql==0.4.2
python -m pip install pyqql-edge==0.4.2

npm view @veristamp/nqql@0.4.2
npm view @veristamp/nqql-edge@0.4.2
npm view qql-wasm@0.4.2
```
