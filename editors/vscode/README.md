...
Extension packaging version is in `package.json` (**0.4.2**). It ships the QQL **1.7** WASM parser from this monorepo (crate version need not match the VSIX version). Note: the checked-in WASM binary may still reflect an older parse surface until rebuilt with `wasm-pack`; TextMate / keyword artifacts stay in sync with the grammar via `qql-grammar-gen generate` without a WASM rebuild, and `npm test` holds the bundle against the full language corpus (canonical formats + invalid-case codes from `language/v1/fixtures`), so staleness fails CI instead of shipping.
